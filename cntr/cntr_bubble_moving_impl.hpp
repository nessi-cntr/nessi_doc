...
/// @private
    template <typename T>
  void Bubble1(herm_matrix_timestep_moving_view<T> &C, int c1, int c2,
  ...
  void Bubble1(herm_matrix_timestep_moving_view<T> &C, int c1, int c2,
    ...
    return Bubble1(C, c1, c2, A, A, a1, a2, B, B, b1, b2);
  }
  /// @private
  template <typename T>
  void Bubble1(herm_matrix_timestep_moving_view<T> &C, herm_matrix_timestep_moving_view<T> &A,
		      herm_matrix_timestep_moving_view<T> &Acc, herm_matrix_timestep_moving_view<T> &B,
		      herm_matrix_timestep_moving_view<T> &Bcc) {
    return Bubble1(C, 0, 0, A, Acc, 0, 0, B, Bcc, 0, 0);
  }
  /// @private
  template <typename T>
  void Bubble1(herm_matrix_timestep_moving_view<T> &C, herm_matrix_timestep_moving_view<T> &A,
		      herm_matrix_timestep_moving_view<T> &B) {
    return Bubble1(C, A, A, B, B);
  }

  template <class GGC, class GGA, class GGB>
  void Bubble1(GGC &C, int c1, int c2, GGA &A, GGA &Acc, int a1, int a2, GGB &B,
		      GGB &Bcc, int b1, int b2) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> acctmp(Acc);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> bcctmp(Bcc);
    Bubble1(ctmp, c1, c2, atmp, acctmp, a1, a2, btmp, bcctmp, b1, b2);
  }
  template <class GGC, class GGA, class GGB>
  void Bubble1(GGC &C, int c1, int c2, GGA &A, int a1, int a2, GGB &B, int b1,
		      int b2) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    Bubble1(ctmp, c1, c2, atmp, a1, a2, btmp, b1, b2);
  }
  template <class GGC, class GGA, class GGB>
  void Bubble1(GGC &C, GGA &A, GGA &Acc, GGB &B, GGB &Bcc) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> acctmp(Acc);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> bcctmp(Bcc);
    Bubble1(ctmp, atmp, acctmp, btmp, bcctmp);
  }
  template <class GGC, class GGA, class GGB> void Bubble1(GGC &C, GGA &A, GGB &B) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    Bubble1( ctmp, atmp, btmp);
  }
...
/// @private
  ...
  /// @private
  template <typename T>
  void Bubble2(herm_matrix_timestep_moving_view<T> &C, int c1, int c2,
		      herm_matrix_timestep_moving_view<T> &A, herm_matrix_timestep_moving_view<T> &Acc, int a1,
		      int a2, herm_matrix_timestep_moving_view<T> &B, herm_matrix_timestep_moving_view<T> &Bcc,
		      int b1, int b2) {
    int tc=C.tc();
  ...
  void Bubble2(herm_matrix_timestep_moving_view<T> &C, int c1, int c2,
		      herm_matrix_timestep_moving_view<T> &A, int a1, int a2,
		      herm_matrix_timestep_moving_view<T> &B, int b1, int b2) {
    return Bubble2(C, c1, c2, A, A, a1, a2, B, B, b1, b2);
  }
  /// @private
  template <typename T>
  void Bubble2(herm_matrix_timestep_moving_view<T> &C, herm_matrix_timestep_moving_view<T> &A,
		      herm_matrix_timestep_moving_view<T> &Acc, herm_matrix_timestep_moving_view<T> &B,
		      herm_matrix_timestep_moving_view<T> &Bcc) {
    return Bubble2(C, 0, 0, A, Acc, 0, 0, B, Bcc, 0, 0);
  }
  /// @private
  template <typename T>
  void Bubble2(herm_matrix_timestep_moving_view<T> &C, herm_matrix_timestep_moving_view<T> &A,
		      herm_matrix_timestep_moving_view<T> &B) {
    return Bubble2(C, A, A, B, B);
  }

  template <class GGC, class GGA, class GGB>
  void Bubble2(GGC &C, int c1, int c2, GGA &A, GGA &Acc, int a1, int a2, GGB &B,
		      GGB &Bcc, int b1, int b2) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> acctmp(Acc);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> bcctmp(Bcc);
    Bubble2(ctmp, c1, c2, atmp, acctmp, a1, a2, btmp, bcctmp, b1, b2);
  }
  template <class GGC, class GGA, class GGB>
  void Bubble2(GGC &C, int c1, int c2, GGA &A, int a1, int a2, GGB &B, int b1,
		      int b2) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    Bubble2(ctmp, c1, c2, atmp, a1, a2, btmp, b1, b2);
  }
  template <class GGC, class GGA, class GGB>
  void Bubble2(GGC &C, GGA &A, GGA &Acc, GGB &B, GGB &Bcc) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> acctmp(Acc);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> bcctmp(Bcc);
    Bubble2(ctmp, atmp, acctmp, btmp, bcctmp);
  }
  template <class GGC, class GGA, class GGB> void Bubble2(GGC &C, GGA &A, GGB &B) {
    herm_matrix_timestep_moving_view<typename GGC::scalar_type> ctmp(C);
    herm_matrix_timestep_moving_view<typename GGA::scalar_type> atmp(A);
    herm_matrix_timestep_moving_view<typename GGB::scalar_type> btmp(B);
    Bubble2(ctmp, atmp, btmp);
  }  
}//namespace cntr
