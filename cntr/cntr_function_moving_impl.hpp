...
*/
  ...
  function_moving<T>::function_moving(int tc,int size1){
    ...
    if(tc_==-1){
        ...
        for(int t=0;t<=tc_;t++) value_[t]=data_+t*element_size_;
    }
  }
...
*/
  ...
  void function_moving<T>::get_value(int i,Matrix &M){
    assert(0<=i && i<=tc());
    cplx *x;
    x=ptr(i);
...
* <!-- ========= -->
*
* > Store the `function_moving` data to a file. It will be stored in txt form.
*
* <!-- ARGUMENTS
*      ========= -->
*
...
/*
		get_value(i, M);
		std::cout << i << ":" << M.rows() << "," << M.cols() << std::endl;
		std::cout << M << std::endl;
*/
      }
    }
...
* > The final value `value[tc]` is shiftes cyclically to `value[0]`
*
* <!-- ARGUMENTS
*      ========= -->
*
...
*/
  template <typename T> 
  void function_moving<T>::set_from_function_backward(function<T>& f, int tstp){
  ...
  template <typename T>
  void function_moving<T>::set_from_function_forward(int tstp,function<T>& f){
    assert(tc_<=tstp && tstp <= f.nt());
    for(int i=0;i<=tc_;i++){
      MatrixXcd tmp;
