...
namespace cntr{
  /// @private
  template < typename T >
  void get_kinetic_trunc_timestep(double &kin,herm_matrix_moving<T> &G,herm_matrix_moving<T> &D,integration::Integrator<T> &I, T h){
    typedef std::complex<T> cplx;
    int k=I.get_k(),k1=k+1;
    cplx *dtemp,*gtemp,*stemp,cplx_one;
    int l;
    int size1=G.size1();
    int tc=G.tc();
    ...
    std::cout<<"size1, sg : "<<size1<<" "<<sg<<std::endl;

    stemp = new cplx [sg];
    gtemp = new cplx [sg];
    dtemp = new cplx [sg];
    element_set_zero<T,LARGESIZE>(size1,stemp);
    for (l=0;l<=k;l++){
        element_conj<T,LARGESIZE>(size1,gtemp,G.retptr(0,l));
        element_minusconj<T,LARGESIZE>(size1,dtemp,G.lesptr(0,l));
        element_incr<T,LARGESIZE>(size1,stemp,h*I.gregory_omega(l),gtemp,D.lesptr(0,l));
        element_incr<T,LARGESIZE>(size1,stemp,h*I.gregory_omega(l),dtemp,D.retptr(0,l));
    }
    for (l=k+1;l<tc-k;l++){
        element_conj<T,LARGESIZE>(size1,gtemp,G.retptr(0,l));
        element_minusconj<T,LARGESIZE>(size1,dtemp,G.lesptr(0,l));
        element_incr<T,LARGESIZE>(size1,stemp,h,gtemp,D.lesptr(0,l));
        element_incr<T,LARGESIZE>(size1,stemp,h,dtemp,D.retptr(0,l));
    }
    for (l=tc-k;l<=tc;l++){
        element_conj<T,LARGESIZE>(size1,gtemp,G.retptr(0,l));
        element_minusconj<T,LARGESIZE>(size1,dtemp,G.lesptr(0,l));
        element_incr<T,LARGESIZE>(size1,stemp,h*I.gregory_omega(tc-l),gtemp,D.lesptr(0,l));
        element_incr<T,LARGESIZE>(size1,stemp,h*I.gregory_omega(tc-l),dtemp,D.retptr(0,l));
    }

    for (l=0;l<sg;l=l+size1+1){
        std::cout<<" l: "<<l<<std::endl;
        kin = kin + stemp[l].imag();
    }
    delete [] stemp;
    delete [] gtemp;
    delete [] dtemp;
...
/// @private
  ...
  void dyson_timestep_dispatch_les(herm_matrix_moving<T> &G,herm_matrix_moving<T> &Sigma,function_moving<T> &eps,T mu, integration::Integrator<T> &I, T h){
    ...
    for(p=0;p<=k+1;p++) diffw[p]=I.bd_weights(p)*cplx_i/h; // use BD(k+1!!)
    for(i=0;i>=tc;i++){
        std::cout<<" Gles: "<<*G.lesptr(1,i)<<" for i= "<<i<<std::endl;
    }
